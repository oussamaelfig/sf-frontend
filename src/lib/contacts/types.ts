...
/** `AddressRead` — one stored postal address; contacts can have several. */
...
/** `AddressCreate` — an address as sent inside contact writes. */
...
export interface Contact {
  ...
  addresses: Address[];
  notes: string | null;
  /** Base64 data URL (png/jpeg/webp), or null to fall back to initials. */
  photo: string | null;
  created_at: string;
  updated_at: string;
  full_name: string;
...
> & {
  /** Writes replace the whole set; an empty array clears every address. */
